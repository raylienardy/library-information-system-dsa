...
class BookRepo:
    ...
    def add(book: Book) -> int:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("INSERT INTO books (title, author, tags, copies) VALUES (?, ?, ?, ?)", book.to_row())
        conn.commit()
    ...
    def update(book: Book) -> bool:
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("UPDATE books SET title=?, author=?, tags=?, copies=? WHERE id=?",
    ...
    def search(term: str) -> List[Book]:
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("SELECT * FROM books WHERE title LIKE ? OR author LIKE ? OR tags LIKE ?", (like, like, like))
        rows = cur.fetchall()
        conn.close()
        return [Book.from_row(r) for r in rows]
...
class TransactionRepo:
    ...
    def add(user_id: int, book_id: int, action: str) -> int:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute("INSERT INTO transactions (user_id, book_id, action) VALUES (?, ?, ?)", (user_id, book_id, action))
        conn.commit()
        tid = cur.lastrowid
        conn.close()
        return tid
