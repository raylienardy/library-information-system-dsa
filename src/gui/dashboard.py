...
def delete_selected(lb):
    sel = lb.curselection()
    if not sel:
        messagebox.showwarning("Pilih", "Pilih buku")
        return
    entry = lb.get(sel[0])
    book_id = int(entry.split("|", 1)[0].strip())
...
def borrow_selected(lb):
    ...
    try:
        ...
        messagebox.showerror("Pinjam", str(e))
        return
    messagebox.showinfo(
        "Antrian",
        f"Buku id={book_id} ditambahkan ke antrian. Posisi: {controller.get_queue_size()}",
    )
...
def open_dashboard():
    ...
    tk.Button(top, text="Cari", command=lambda: search_and_show(lb, search_var.get())).pack(
        side=tk.LEFT
    )
    tk.Button(
        top, text="Segarkan", command=lambda: (controller.init_app(), refresh_listbox(lb))
    ...
    tk.Button(btn_frame, text="Tambah Buku", command=lambda: add_book_and_refresh(lb)).pack(
        side=tk.LEFT, padx=4
    )
    tk.Button(btn_frame, text="Edit", command=lambda: edit_selected(lb)).pack(side=tk.LEFT, padx=4)
    tk.Button(btn_frame, text="Hapus", command=lambda: delete_selected(lb)).pack(
        side=tk.LEFT, padx=4
    )
    tk.Button(btn_frame, text="Pinjam (Antri)", command=lambda: borrow_selected(lb)).pack(
        side=tk.LEFT, padx=4
    )
    tk.Button(btn_frame, text="Proses Antrian", command=lambda: process_queue_now(lb)).pack(
        side=tk.LEFT, padx=4
    )
    tk.Button(btn_frame, text="Undo Terakhir", command=lambda: undo_last_action(lb)).pack(
        side=tk.LEFT, padx=4
    )
    tk.Button(
        btn_frame, text="Rekomendasi", command=lambda: show_recommendations_for_selected(lb)
    ).pack(side=tk.LEFT, padx=4)
    tk.Button(
        btn_frame, text="Logout", command=lambda: (controller.do_logout(), win.destroy())
