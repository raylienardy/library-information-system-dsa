...
def main():
    ...
    username_var = tk.StringVar(); tk.Entry(frm, textvariable=username_var).grid(row=0, column=1)
    tk.Label(frm, text="Password").grid(row=1, column=0, sticky="w", padx=6, pady=4)
    password_var = tk.StringVar(); tk.Entry(frm, textvariable=password_var, show="*").grid(row=1, column=1)

    btn_frame = tk.Frame(root); btn_frame.pack(pady=12)
